...
for root, subdir, files in os.walk(walk_dir): #redo in listdir
	...
	for i in range(len(subdir)):
		...
		if root == sys.argv[1]:
			...
			if exit_code == 0:
				command = path + "/WordlerProTest"
				os.system(command)
				command = "cp " + path + "/*.cpp ./"
				os.system(command)
				command = "cp " + path + "/*.h ./"
				os.system(command)
				command = "gcov " + path + "/WordlerPro.cpp"
				os.system(command)
				command = "python cov_fut.py WordlerPro.cpp.gcov >> cov_fut_results"
				os.system(command)
				command = "rm *.cpp *.gcov *.h"
				os.system(command)
