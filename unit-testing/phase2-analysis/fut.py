...
function_regx = re.compile("[a-zA-Z0-9\_]+([\.]|->)[a-zA-Z0-9\_]+[(].*[)]")
...
with my_ifile:
	for cur_line in my_ifile:
		...
		cur_line = cur_line.replace("c_str","")
		cur_line = cur_line.replace("size()","")
		cur_line = cur_line.replace("length()","")
		cur_line = cur_line.replace("push_back","")
		cur_line = cur_line.replace("begin()","")
		cur_line = cur_line.replace("end()","")
		cur_line = cur_line.replace("thatthegivensecretis","")
		cur_line = cur_line.replace("print_secret","")
		cur_line = cur_line.replace("getWord","")
		cur_line = cur_line.replace("get_secret","")
		...
		if single_comment == False and multi_comment == False:
			...
			if cur_line.find("TEST(") >= 0:
				...
				print "Current line: " + str(num_line)
			...
			if start_of_test:
				...
				if pattern:
					...
					if "." in function:
						...
						function = function[function.find("->")+2:function.find("(")]
