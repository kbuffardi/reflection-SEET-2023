...
def getComplexity(name, line_num):
  output = subprocess.check_output("pmccabe /data/" + name + "/*Test.cpp", shell=True)
  complexity = 0
...
def getLine(source):
  line = re.findall(r'Current line: .+',source)
  if line:
...
try:
  ...
  for i, tester in enumerate(fut_results):
      ...
      for case in cases:
        line = getLine(case)
        complexity = getComplexity(testername, line)
        fut = getFUT(case)
        ...
        if fut not in fut_data[testername]:
          fut_data[testername][fut] = {"line": line,
                                       "cases": 1,
                                       "unique": unique,
          ...
          fut_data[testername][fut] = {"line": 
                                       ...
                                       "coverage": 0,
                                       "complexity": 
                                       fut_data[testername][fut]["complexity"] + complexity}
  ...
  for coverage_output in testers:
    ...
    if fut_data[cov_tester]:
      ...
      fut_data[cov_tester][TOTAL()]["coverage"] = cov
        ...
        fut_data[cov_tester] = {}
        fut_data[cov_tester][TOTAL()] = {}
        fut_data[cov_tester][TOTAL()]["coverage"] = ""
...
try:
  ...
  with newCSVFile(results+".accuracy") as output:
    ...
    output.write("id,function_under_test,true_positives,true_negatives,false_positives," \
                 ...
                 +"fut_assertions,fut_function_calls,coverage,test_complexity\n")
    for tester in dataframe:
      all_tp = 0
      all_tn = 0
      all_fp = 0
      ...
      for function in all_functions:
        ...
        if function not in fut_data[tester]:
          ...
          complexity = fut_data[tester][function]["complexity"]
        all_cases += cases
        all_unique += unique
      ...
      output.write(tester+","+TOTAL()+","+str(all_tp)+","+str(all_tn)+"," \
                   ...
                   +str(all_assertions)+","+str(all_calls)+","+str(coverage)+","+str(all_complexity)+"\n")
    output.close()
